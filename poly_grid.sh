...
if ! echo "$OPTS" | grep "n" > /dev/null ; then
	...
	if [ $HAS_SPLIT -ne 0 ] ; then
		...
		polysplit -i "$LAYER_IN" "$DB" "$TMP_FILE.shp"
...
if [ $HAS_GRID -ne 0 ] ; then
	...
	tile_grid.py -srs "$SRID" $SPLIT_CNT "$TMP_FILE.shp"

	ogr2ogr --config OGR_SQLITE_SYNCHRONOUS OFF -f "SQLite" -gt 65535 -update -append -skipfailures -a_srs "EPSG:$SRID" "$DB" "$TMP_FILE.shp" -nln "grid" -nlt "POLYGON"
...
DELETE FROM $LAYER_OUT WHERE ST_IsEmpty(GEOMETRY) = 1;
...
DELETE FROM $LAYER_OUT WHERE ST_Area(GEOMETRY) < $AREA_LIMIT;
...
DURATION_SPLIT=`expr $SEND_SPLIT - $SSTART`
DURATION_EXT=`expr $SEND_EXT - $SEND_SPLIT`
DURATION_GRID=`expr $SEND_GRID - $SEND_EXT`
DURATION=`expr $SEND - $SEND_GRID`
